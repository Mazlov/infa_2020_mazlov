...
def hands(color, size, x_right_arm, y_right_arm, x_left_arm, y_left_arm):
    line(s, color, (x_left_arm + int(1.4*size),y_left_arm + int(4.85*size)),(x_left_arm + int(0.1*size),y_left_arm + int(0.35*size)), int(0.3*size))
    line(s, color, (x_right_arm - int(1.6*size),y_right_arm + int(4.85*size)),(x_right_arm - int(0.1*size),y_right_arm + int(0.35*size)), int(0.3*size))
...
def head(color, x_r, x_l, y_r, y_l, size):
    circle(s, color, (int((x_right_arm + x_left_arm)/2), int((y_right_eye + y_left_eye)/2 + 0.5*size) ), int(2*size))
...
def body(color, x, y, size):
    ...
    polygon(s, color, [(x - int(2.7*size),y - int(1.2*size)),(x - int(2.7*size),y - int(2.2*size)),
                       (x - int(1.95*size), y - int(2.7*size)), (x - int(1.35*size),y - int(2*size)), (x - int(1.8*size),y-int(1.1*size))])
    polygon(s, color, [(x + int(1.9*size),y - int(1.1*size)),(x + int(2.7*size),y - int(1.4*size)),
                       (x + int(2.7*size), y - int(2.3*size)), (x + int(1.9*size),y - int(2.7*size)), (x + int(1.5*size),y - int(2*size))])

def mouth(color, x, y, size):
    polygon(s, color, [(x,y - int(2.6*size)),(x - int(1.2*size),y - int(3.4*size)), (x + int(1.2*size), y - int(3.4*size))])
    
def nose(color, x, y, size):
    polygon(s, color, [(x,y - int(3.5*size)),(x - int(0.2*size),y - int(3.85*size)), (x + int(0.2*size), y - int(3.85*size))])

def hair(color, size, x, y):
    arc(s,color,pygame.Rect(x -int(1.4*size),y - int(1.7*size), int(4.4*size),int(4.4*size)),3.14/4, 3*3.14/4, int(0.3*size))
